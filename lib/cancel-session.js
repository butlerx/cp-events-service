...
function cancelSession(args, callback) {
  ...
  function loadSessionData(done) {
    seneca.act({ role: plugin, cmd: 'loadSession', id: sessionId }, (err, res) => {
      if (err) return done(err);
      sessionData = res;
      seneca.act({ role: plugin, cmd: 'getEvent', id: sessionData.eventId }, (getErr, event) => {
  ...
  function updateApplicationsStatus(cancelledSession, done) {
    seneca.act({
      ...
      query: { sessionId: cancelledSession.id },
    }, (err, applications) => {
      if (err) return done(err);
  ...
  function updateEventStatus(done) {
    seneca.act({
      ...
      _.each(sessions, (sesh) => {
        if (sesh.status !== 'cancelled') {
          allSessionsCancelled = false;
        }
      });
      if (allSessionsCancelled && eventData.status !== 'cancelled') {
        eventData.status = 'cancelled';
        seneca.act({ role: plugin, cmd: 'saveEvent', eventInfo: eventData }, (error, event) => {
  ...
  function emailSessionAttendees(done) {
    ...
    process.nextTick(() => {
      applicationsData = _.uniq(applicationsData, (application) => {
        return application.userId;
      });
      async.eachSeries(applicationsData, (application, cb) => {
        seneca.act({
          role: 'cd-profiles',
          cmd: 'list',
          ...
          function emailParents(emailCb) {
            if (!_.isEmpty(profile.parents)) {
              async.eachSeries(profile.parents, (parent, sCb) => {
                seneca.act({
                  role: 'cd-users',
                  cmd: 'load',
                  id: parent,
