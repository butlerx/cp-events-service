...
function updateApplicationAttendance(args, callback) {
  ...
  if (!data.date || !moment(data.date).isValid()) {
    return callback(null, { error: 'date is empty or not valid' });
  }
  seneca.act({
    role: plugin,
    cmd: 'loadApplication',
    ...
    if (data.attended) {
      ...
      application.attendance = _.without(
        application.attendance,
        _.find(
          application.attendance,
          attendance => moment(attendance).diff(moment(data.date)) === 0,
        ),
      );
    }
    seneca.act({ role: plugin, cmd: 'saveApplication', application }, (error) => {
      if (error) return callback(null, { ok: false, why: error.message });
